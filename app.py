...
/* Note: the revenue-goal bar is NOT st.progress() — it's custom HTML
   (see goal_html below) specifically because Streamlit's own progress
   bar has an internal DOM structure that isn't reliably targetable via
   CSS across versions; a custom element sidesteps that entirely. */
...
/* Password show/hide toggle and any other icon-buttons living inside a
   text input (BaseWeb renders these as their own element, separate from
   the input box, with their own default-dark background+icon fill). */
...
[data-testid="stTextInput"] svg, [data-baseweb="input"] svg{{
  fill:var(--text-mid) !important; color:var(--text-mid) !important;
}}
...
<div style="margin:2px 0 16px;">
  <div style="font-size:13.5px; color:var(--text-hi); margin-bottom:7px; font-family:'JetBrains Mono',monospace;">
    Revenue goal: ${k['revenue']:,.0f} of ${goal:,.0f} ({pct_pct:.1f}%)
  </div>
  <div style="height:11px; border-radius:6px; background:var(--line); overflow:hidden; border:1px solid var(--line);">
    <div style="height:100%; width:{pct_pct:.2f}%; min-width:{2 if pct_pct > 0 else 0}px;
