...
def build_css(theme: str) -> str:
    """Full CSS for the app. Streamlit's own base theme is fixed at
    startup (config.toml), so this overrides colors broadly enough to
    make both modes look right — a few native widget popups (date
    picker calendar, select dropdown menu) may keep their original
    dark chrome since those render outside normal CSS reach."""
    if theme == "light":
...
[data-testid="stSidebar"]{{ background:var(--bg-panel) !important; border-right:1px solid var(--line); }}
[data-testid="stSidebar"] *{{ color:var(--text-hi); }}
h1,h2,h3,h4{{ font-family:'Space Grotesk','Trebuchet MS',sans-serif !important; color:var(--text-hi) !important; }}
p, span, label, li, [data-testid="stMarkdownContainer"]{{ color:var(--text-hi); }}
[data-testid="stCaptionContainer"], .stCaption, small{{ color:var(--text-mid) !important; }}
...
[data-testid="stMetricValue"]{{ font-family:'JetBrains Mono',monospace !important; color:var(--text-hi) !important; }}

div[data-testid="stExpander"]{{ border:1px solid var(--line); border-radius:10px; background:var(--bg-panel); }}
...
textarea, [data-baseweb="select"] > div{{
...
.stButton button:hover, [data-testid="stFormSubmitButton"] button:hover{{ border-color:var(--cyan); color:var(--cyan); }}
.stButton button[kind="primary"], [data-testid="stFormSubmitButton"] button[kind="primary"]{{
  background:linear-gradient(135deg, var(--cyan), var(--violet)); color:#080B14 !important; border:none; font-weight:600;
}}
...
.hero-logo{{
  ...
  font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:28px; color:#080B14;
  box-shadow:0 0 32px rgba(51,225,224,0.35);
}}
.hero-title{{ font-family:'Space Grotesk',sans-serif; font-size:30px; font-weight:700; color:var(--text-hi); margin-bottom:4px; }}
.hero-tagline{{ font-size:13px; color:var(--text-mid); text-transform:uppercase; letter-spacing:2px; margin-bottom:24px; }}
.feature-row{{ display:flex; justify-content:center; gap:10px; flex-wrap:wrap; margin-bottom:28px; }}
.feature-chip{{
  display:flex; align-items:center; gap:6px; font-size:12px; color:var(--text-mid);
  background:var(--bg-panel); border:1px solid var(--line); border-radius:20px; padding:7px 14px;
}}
.st-key-pulse_login_card{{
  max-width:440px; margin:0 auto; background:linear-gradient(180deg, var(--bg-panel), var(--bg-panel-2));
