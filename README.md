...
├── secrets.toml.example        # copy into Streamlit Cloud "Secrets" for real logins
...
The app is behind a login screen (`streamlit-authenticator`).

- **No setup done yet?** It falls back to a demo account —
  username `admin`, password `admin123`. Fine for trying it out, not
  for sharing publicly.
- **Real users:** copy `secrets.toml.example`, fill in your own
  usernames/passwords, and paste it into:
  - **Streamlit Cloud:** your app → Settings → Secrets
  - **Local dev:** a `.streamlit/secrets.toml` file (already
    git-ignored, never commit real passwords)

  Add as many `[credentials.usernames.<id>]` blocks as you need users.
...
- **Light/Dark theme toggle** — top-right on the login page and in the
  sidebar once signed in. Note: a few native Streamlit widget popups
  (date-picker calendar, select dropdown menu) keep their original
  dark styling regardless of the toggle — a Streamlit platform
  limitation (base theme is fixed at startup), not a bug in the app.
- **Redesigned login page** — branded landing screen (logo, tagline,
  feature highlights) with a centered card, instead of a bare form.
- **Login** — multi-user access via `streamlit-authenticator`.
...
- **Revenue forecast** — a `scikit-learn` linear-trend projection
  (7/14/30 periods ahead) with a 95% band, on whatever range/filters
  are currently active.
- Anomaly detection (z-score > 3σ) and big-order alerts, shown as an
  in-page alert log plus `st.toast()` popups.
- **Email alerts** — optional SMTP config in the sidebar; critical
  anomalies are emailed to you automatically when enabled (use an
  app-specific password, e.g. a Gmail App Password — never your real
  account password).
- **Reports** — one-click **PDF** summary (KPIs, top products, region/
  category breakdown) and a multi-sheet **Excel workbook**
  (Summary, By Region, By Category, Top Products, Transactions).
